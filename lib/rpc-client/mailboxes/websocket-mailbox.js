...
//          console.log("ws received message unzip len = " + result.length);
//          console.log("ws received message unzip = " + result);
          msg = result;
        });
      }
      else{
//        console.log("ws rpc received message = " + data);
        msg = data;
      }
...
var doSend = function(socket, dataObj) {
  ...
  if(str.length > DEFAULT_ZIP_LENGTH){//send zip binary
    zlib.gzip(str, function (err, result) {
      if(!!err){
        console.warn('ws rpc send message error: %j', err.stack);
        return;
      }
      //console.log("ws rpc send message by zip compress, buffer len = " + result.length);
      socket.send(result);
    });
  }
